...
class VillageTest extends AnyFunSuite {
  ...
  test("Village has at least one shop type building") {
    ...
    assert(
      shopCount >= 1,
      s"Village should have at least 1 shop/healer, but has $shopCount"
    )
  }
  ...
  test("Village buildings meet minimum size requirements") {
    ...
    village.buildings.foreach { building =>
      assert(
        building.width >= 5,
        s"Building width should be at least 5, but is ${building.width}"
      )
      assert(
        building.height >= 5,
        s"Building height should be at least 5, but is ${building.height}"
      )
    }
  }
  ...
  test("Village tiles include all building tiles") {
    ...
    village.buildings.foreach { building =>
      building.tiles.foreach { case (point, tileType) =>
        assert(
          village.tiles.contains(point),
          s"Village should contain building tile at $point"
        )
      }
    }
  }
  ...
  test("Village walls include all building walls") {
    ...
    village.buildings.foreach { building =>
      building.walls.foreach { wallPoint =>
        assert(
          village.walls.contains(wallPoint),
          s"Village should contain building wall at $wallPoint"
        )
      }
    }
  }
  ...
  test("Village.shopBuilding returns the shop building") {
    ...
    assert(
      village.shopBuilding.isShop,
      "Shop building should have isShop = true"
    )
  }
  ...
  test("Village.entrances returns all building entrances") {
    ...
    assert(
      village.entrances.length == village.buildings.length,
  ...
  test("Village.findVillageLocation avoids dungeons") {
    ...
    assert(
      distance > 20,
      s"Village at $villageLocation should be far from dungeon center, but distance is $distance"
    )
  }
  ...
  test("Village.findVillageLocation is within world bounds") {
    ...
    assert(
      villageLocation.x >= worldMinX + margin && villageLocation.x <= worldMaxX - margin,
      s"Village X at ${villageLocation.x} should be within world bounds"
    )
    assert(
      villageLocation.y >= worldMinY + margin && villageLocation.y <= worldMaxY - margin,
  ...
  test("Village with different seeds generates different layouts") {
    ...
    assert(
      different,
      "Villages with different seeds should generate different layouts"
    )
  }
  ...
  test("Village generates paths connecting buildings") {
    val village = Village.generateVillage(Point(0, 0), seed = 88888)

    assert(village.paths.nonEmpty, "Village should have generated paths")

    // Check that paths are included in tiles
    village.paths.foreach { pathPoint =>
      assert(
        village.tiles.contains(pathPoint),
        s"Village tiles should contain path point $pathPoint"
      )
