...
class DungeonEntrancePlacementTest extends AnyFunSuite {
  ...
  ignore("Dungeon entrance rooms face toward player spawn") {
    ...
    mutatedWorld.dungeons.zipWithIndex.foreach { case (dungeon, idx) =>
      ...
      println(
        s"  Rooms closer to player than entrance: ${roomsCloserThanEntrance.size}"
      )
      ...
      if (roomsCloserThanEntrance.nonEmpty) {
        ...
        roomsCloserThanEntrance.take(3).foreach { room =>
          val roomDist =
            math.sqrt(math.pow(room.x - 0, 2) + math.pow(room.y - 0, 2))
          println(
            s"    Room $room at distance $roomDist (entrance: $entranceDistance)"
          )
        }
      }

      assert(
        roomsCloserThanEntrance.isEmpty,
  ...
  test("Dungeons respect 10-tile exclusion zone around player") {
    ...
    println(s"\n=== Exclusion Zone Test ===")
    println(
      s"Exclusion radius: $exclusionRadius tiles = ${exclusionRadius / roomSize} rooms"
    )
    println(s"Generated ${mutatedWorld.dungeons.size} dungeons")
    ...
    mutatedWorld.dungeons.zipWithIndex.foreach { case (dungeon, idx) =>
      ...
      assert(
        tilesInExclusionZone.isEmpty || minDistance >= exclusionTileRadius * 0.9,
  ...
  test("Dungeons have varied sizes") {
    ...
    mutatedWorld.dungeons.zipWithIndex.foreach { case (dungeon, idx) =>
      ...
      println(
        s"  Dungeon $idx: ${dungeon.roomGrid.size} rooms, bounds: ${bounds.roomWidth}x${bounds.roomHeight}"
      )
    }
