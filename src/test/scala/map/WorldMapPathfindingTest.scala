...
class WorldMapPathfindingTest extends AnyFunSuite {
  ...
  test("PathGenerationMutator uses pathfinding to avoid dungeon walls") {
    ...
    val worldWithTerrain = terrainMutator.mutateWorld(
      WorldMap(
        tiles = Map.empty,
        dungeons = Seq.empty,
        paths = Set.empty,
    ...
    assert(
      pathReachesDungeon,
      s"Path should reach near dungeon entrance at $dungeonEntrance"
    )
    ...
    val entranceArea = (for {
      dx <- -2 to 2
      dy <- -2 to 2
    } yield Point(realEntrance.x + dx, realEntrance.y + dy)).toSet
    ...
    assert(
      wallsOutsideEntrance.isEmpty || wallsOutsideEntrance.size < 5,
      s"Paths should avoid dungeon walls away from entrance. Found ${wallsOutsideEntrance.size} wall intersections"
    )

    println(
      s"✓ Path successfully generated with ${worldWithPaths.paths.size} tiles"
    )
    println(s"✓ Path reaches dungeon entrance at $dungeonEntrance")
    println(
      s"✓ Path avoids ${dungeon.walls.size} dungeon walls (${pathsInWalls.size} path tiles in dungeon)"
    )
  }
  ...
  test("PathGenerationMutator uses pathfinding to avoid building walls") {
    ...
    // Verify paths were created
    assert(
      worldWithPaths.paths.nonEmpty,
      "Paths should be generated to village"
    )
    ...
    assert(
      entranceReached,
      "Path should reach at least one village building entrance"
    )
    ...
    assert(
      wallsOutsideEntrances.isEmpty || wallsOutsideEntrances.size < 5,
    ...
    println(
      s"✓ Path reaches village buildings (${village.buildings.size} buildings)"
    )
    println(
      s"✓ Path avoids ${village.walls.size} building walls (${pathsInWalls.size} path tiles in walls)"
    )
  }
  ...
  test("PathGenerationMutator creates bridges over rivers") {
    ...
    // Add a river manually (vertical stripe)
    val riverTiles = (for {
      y <- -10 to 10
      x <- -2 to 0 // 3 wide river
    } yield Point(x, y) -> TileType.Water).toMap

    val worldWithRiver = worldWithTerrain.copy(
      tiles = worldWithTerrain.tiles ++ riverTiles
    )
    ...
    if (worldWithPaths.bridges.nonEmpty) {
      ...
      val bridgesNotInPaths = worldWithPaths.bridges -- worldWithPaths.paths
      assert(
        bridgesNotInPaths.isEmpty,
        s"Bridges should be part of paths. Found orphans: $bridgesNotInPaths"
      )

      // Verify bridges are on WATER tiles (either manual river or generated water)
      val bridgesNotOnWater = worldWithPaths.bridges.filterNot(p =>
        worldWithRiver.tiles.get(p).contains(TileType.Water)
      )
      assert(
        bridgesNotOnWater.isEmpty,
        s"Bridges should be on water tiles. Found invalid bridges: $bridgesNotOnWater"
      )
  ...
  test("paths prefer straight lines and minimize corners") {
    ...
    assert(
      pathSet.contains(start) || pathSet.exists { p =>
    ...
    assert(
      pathSet.contains(target) || pathSet.exists { p =>
