...
class VerifyPlayerToDungeonPathTest extends AnyFunSuite {
  test("Verify player-to-dungeon path exists and connects properly") {
    ...
    val dungeonEntranceTileX = dungeon.startPoint.x * map.Dungeon.roomSize + map.Dungeon.roomSize / 2
    val dungeonEntranceTileY = dungeon.startPoint.y * map.Dungeon.roomSize + map.Dungeon.roomSize / 2
