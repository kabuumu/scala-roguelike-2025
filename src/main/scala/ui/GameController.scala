...
case class GameController(
    ...
    cachedWorldMapView: Option[indigo.Batch[indigo.SceneNode]] = None
) {
  ...
  def init(): GameController = {
    copy(gameState =
      gameState.updateEntity(
        ...
        gameState.playerEntity.update[SightMemory](
          _.update(gameState, entity = gameState.playerEntity)
        )
      )
    )
  }
    ...
    if (delta >= ticksPerSecond / framesPerSecond) {
      val (newUiState, optAction) = optInput match {
        case Some(input) if delta >= ticksPerSecond / allowedActionsPerSecond =>
          uiState match {
            case _: UIState.MainMenu | UIState.GameOver =>
              InputHandler.handleInput(
                input,
                uiState,
                gameState
              ) // MainMenu and GameOver don't need ready check
            case _ if gameState.playerEntity.isReady =>
              InputHandler.handleInput(
                input,
                uiState,
                gameState
              ) // Other states need ready check
            case _ => (uiState, None)
          }
        case _ =>
          (uiState, None)
      ...
      GameStateTransitions
        ...
        .getOrElse {
          ...
          val newGameState =
            gameState.updateWithSystems(
              optAction
                .map(action => InputEvent(gameState.playerEntity.id, action))
                .toSeq
            )
