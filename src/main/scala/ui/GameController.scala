...
case class GameController(uiState: UIState, gameState: GameState, lastUpdateTime: Long = 0) {
  ...
  def update(optInput: Option[Input], currentTime: Long): GameController = {
    ...
    if (delta >= ticksPerSecond / framesPerSecond) {
      ...
      handleSpecialStateTransitions(newUiState, optAction, currentTime)
        .getOrElse {
          // Continue with normal flow
          performAutosave(optAction)
          ...
          handlePlayerDeathTransition(newGameState, currentTime)
            .getOrElse {
  ...
  private def handleSpecialStateTransitions(newUiState: UIState, optAction: Option[InputAction], currentTime: Long): Option[GameController] = {
      ...
      case (UIState.Move, Some(InputAction.LoadGame)) =>
        ...
        SaveGameSystem.loadGame() match {
          ...
          case scala.util.Failure(exception) =>
            ...
            Some(GameController(uiState, errorGameState, currentTime))
        }
      case _ => None // Continue with normal flow
    }
  }

  private def performAutosave(optAction: Option[InputAction]): Unit = {
  ...
  private def handlePlayerDeathTransition(newGameState: GameState, currentTime: Long): Option[GameController] = {
