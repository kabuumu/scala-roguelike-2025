...
/** Handles targeting logic for finding enemies and action targets. This
  * contains utility methods for range checking and target selection.
  */
object GameTargeting {
    ...
    gameState.entities
      .filter { enemyEntity =>
        ...
        gameState
          ...
          .contains(enemyEntity.position)
        &&
        enemyEntity.isAlive
      }
      .sortBy(enemyEntity =>
        enemyEntity.position
          .getChebyshevDistance(gameState.playerEntity.position)
      )
  }
  ...
  def nearbyActionTargets(gameState: GameState): Seq[ActionTarget] = {
    ...
    val adjacentPositions =
      Direction.values.map(dir => playerPosition + Direction.asPoint(dir)).toSet
    ...
    val tradeTargets = gameState.entities
      ...
      .filterNot(_.hasConversation) // Prefer conversation if available
      .map(TradeTarget.apply)

    // Get nearby conversation targets
    val conversationTargets = gameState.entities
      .filter(e => adjacentPositions.contains(e.position))
