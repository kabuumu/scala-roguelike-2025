...
/** Represents a single building in a village. Buildings are rectangular
  * structures with configurable dimensions.
  *
  * @param location
  *   The room coordinate where the building is located
  * @param width
  *   Width of the building in tiles (internal space)
  * @param height
  *   Height of the building in tiles (internal space)
  * @param buildingType
  *   The type of building (Healer, Shop, etc.)
  */
case class Building(
    location: Point,
    width: Int,
  ...
  /** Calculate the entrance tile position (middle of one wall). The entrance
    * faces the origin (0, 0) for easier access.
    */
  ...
  /** Generate all tiles for the building. Creates a rectangular building with
    * brick walls and floor inside.
    */
  lazy val tiles: Map[Point, TileType] = {
    ...
    val buildingTiles = for {
      ...
      val tileType = if (buildingType == BuildingType.Farmland) {
        TileType.Farmland
  ...
  def bounds: (Point, Point) =
    (location, Point(location.x + width, location.y + height))
}
