...
/** Trait for world generation mutators. Similar to DungeonMutator, each mutator
  * performs a specific transformation on a WorldMap. This allows for
  * extensible, composable world generation.
  */
trait WorldMutator {
...
/** Mutator that places dungeons in the world. Analyzes the world map to
  * determine optimal dungeon placement.
  *
  * @param playerStart
  *   The player's starting position (default: Point(0,0))
  * @param seed
  *   Random seed for deterministic generation
  * @param exclusionRadius
  *   Minimum distance in tiles from player start where dungeons cannot be
  *   placed
  * @param numDungeonsOverride
  *   Optional override for number of dungeons to place
  */
class DungeonPlacementMutator(
  ...
  override def mutateWorld(worldMap: WorldMap): WorldMap = {
    val dungeonConfigs = analyzeMapAndCalculateDungeons(worldMap)
    val dungeons =
      dungeonConfigs.map(config => DungeonGenerator.generateDungeon(config))
  ...
  /** Analyzes the world map and calculates dungeon configurations. Places
    * dungeons away from player start with varied sizes and entrance
    * orientations.
    */
  private def analyzeMapAndCalculateDungeons(
      worldMap: WorldMap
  ): Seq[DungeonConfig] = {
    ...
    val configs = (0 until numDungeons).flatMap { i =>
      ...
      val regionCenterY =
        bounds.minRoomY + row * regionHeight + regionHeight / 2
      ...
      val effectiveExclusionRooms = if (worldArea < 200) {
        math.max(
          0,
          exclusionRooms - 1
        ) // Reduce exclusion by 1 room for small worlds
      } else {
        exclusionRooms
      }
      ...
      if (centerDistance < effectiveExclusionRooms) {
        ...
        val entranceSide = if (yDiff > xDiff) {
          ...
          if (dungeonCenterY > playerRoomY)
            Direction.Up // Dungeon below player: face Up
          else Direction.Down // Dungeon above player: face Down
        } else {
          // X difference is greater: entrance faces toward player horizontally
          if (dungeonCenterX > playerRoomX)
            Direction.Left // Dungeon right of player: face Left
        ...
        Some(
          DungeonConfig(
            bounds =
              MapBounds(clampedMinX, clampedMaxX, clampedMinY, clampedMaxY),
            seed = seed + i,
    ...
    if (configs.isEmpty && numDungeons > 0) {
      ...
      val dungeonMaxX = math.min(
        bounds.maxRoomX,
        dungeonMinX + math.max(5, bounds.roomWidth / 2)
      )
      val dungeonMinY = math.max(bounds.minRoomY, playerRoomY + offsetY)
      val dungeonMaxY = math.min(
        bounds.maxRoomY,
        dungeonMinY + math.max(5, bounds.roomHeight / 2)
      )

      Seq(
        DungeonConfig(
          bounds =
            MapBounds(dungeonMinX, dungeonMaxX, dungeonMinY, dungeonMaxY),
          seed = seed,
...
/** Mutator that places a village at the player spawn point (origin). This
  * ensures the player starts in a safe settlement. Must be placed BEFORE
  * DungeonPlacementMutator so dungeons avoid this area.
  *
  * @param spawnPoint
  *   The player's spawn location (typically Point(0,0))
  * @param seed
  *   Random seed for deterministic generation
  */
...
/** Mutator that places villages in the world. Villages are collections of 3-5
  * buildings, with one building being a shop.
  *
  * @param worldBounds
  *   The bounds of the world
  * @param numVillages
  *   Total number of villages desired (including spawn village)
  * @param seed
  *   Random seed for deterministic generation
  */
class VillagePlacementMutator(
  ...
  override def mutateWorld(worldMap: WorldMap): WorldMap = {
    ...
    val villages = (0 until additionalVillages).map { i =>
      val villageLocation = Village.findVillageLocation(
        dungeonBounds,
        worldBounds,
        preferredDistance = 30 + i * 50
      )
    ...
    worldMap.copy(
      ...
      shop = villages.headOption
        .map { village =>
          val shopBuilding = village.shopBuilding
          Shop(
            location = Point(
              shopBuilding.location.x / 10,
              shopBuilding.location.y / 10
            ), // Approximate room coords
            size = 10
          )
        }
...
/** Mutator that creates dirt paths between key locations (spawn, dungeons,
  * villages). If paths cross rivers, bridges are placed instead of dirt tiles.
...
class PathGenerationMutator(startPoint: Point) extends WorldMutator {
  override def mutateWorld(worldMap: WorldMap): WorldMap = {
    ...
    val dungeonConnectingPaths = worldMap.dungeons.flatMap { dungeon =>
      val entranceDoor =
        Dungeon.getEntranceDoor(dungeon.startPoint, dungeon.entranceSide)
      val approachTile =
        Dungeon.getApproachTile(dungeon.startPoint, dungeon.entranceSide)
...
/** Mutator that ensures walkable paths by clearing tree clusters. This is an
  * optional mutator that can be applied if needed.
  */
