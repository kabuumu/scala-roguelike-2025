...
object Elements {
  ...
  def equipmentPaperdoll(
    ...
    // Title
    val title =
      text("Equipment", paperdollX, paperdollY - (defaultBorderSize * 3))
    ...
    val drText =
      text(s"DR: $totalDamageReduction", paperdollX + defaultBorderSize, statsY)
    val dmgText = text(
  ...
  def messageWindow(model: GameController): Batch[SceneNode] = {
    ...
    val messageContent = model.uiState match {
      case UIState.Move =>
        ...
        val actionTargets =
          ui.GameTargeting.nearbyActionTargets(model.gameState)
    ...
    val wrappedLines =
      wrapText(messageContent, messageWidth) // Approximate character width
  ...
  def gameOverScreen(
    ...
    val itemsText =
      text(s"Items used: $itemsUsed", uiXOffset, statsY + spriteScale)
  ...
  def conversationWindow(
    ...
    model.uiState match {
      case interactionState: UIState.InteractionState =>
        ...
        val nameText = text(entityName, messageX, messageY)
        ...
        val wrappedText = wrapText(interactionState.message, maxLineChars)
        val textLines = wrappedText.zipWithIndex.map { case (line, idx) =>
          text(line, messageX, textY + (idx * (spriteScale / 2)))
        }
        ...
        val optionElements = interactionState.options.zipWithIndex.flatMap {
          case ((optionText, action), index) =>
            ...
            highlight.toSeq :+ text(displayText, textX, optionY)
        }
  ...
  private def renderScrollingMenu(
      title: String,
      options: Seq[String],
      selectedIndex: Int,
    ...
    val titleY = spriteScale * 2
    val titleNode = text(title, titleX, titleY)
    ...
    val listNodes = (minIndex to maxIndex).flatMap { index =>
      ...
      val optionText = text(textStr, menuX, y) match {
        case t: Text[?] =>
  ...
  def debugPerkSelection(model: GameController): Batch[SceneNode] = {
    model.uiState match {
      case uiState: UIState.DebugGivePerkSelect =>
        ...
        val detailNodes = Seq(
          text(perk.name, detailX, detailY),
          text("- " + perk.description, detailX, detailY + spriteScale * 2),
          text(
  ...
  def debugItemSelection(
    ...
    model.uiState match {
      case uiState: UIState.DebugGiveItemSelect =>
        ...
        // Use existing item display logic but try to shift it or just let it overlay
        // renderBuyItemDisplay positions center-ish.
