 ...
 * @param effects The effects the item has when activated
...
object UsableItem {
  ...
  object builders {
    ...
    def healingPotion(healAmount: Int = 40): UsableItem =
      UsableItem(Targeting.Self, Seq(ItemEffect.Heal(healAmount)), consumeOnUse = true)
    
    /** Create an enemy-targeting bow that requires arrows */
    def bow(damage: Int = 8, ammoType: String = "Arrow"): UsableItem =
      UsableItem(Targeting.EnemyActor, Seq(ItemEffect.CreateProjectile(damage)), consumeOnUse = false, ammo = Some(ammoType))
    ...
    def fireballScroll(explosionRadius: Int = 2, explosionDamage: Int = 30): UsableItem =
      UsableItem(
        ...
        Seq(ItemEffect.CreateProjectile(0, Some(ExplosionEffect(explosionRadius, explosionDamage)))),
        consumeOnUse = true
      )
  }
}