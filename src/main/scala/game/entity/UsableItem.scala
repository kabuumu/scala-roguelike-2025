...
import game.event.{Event, HealEvent, CreateProjectileEvent}
 ...
 * Now uses Events directly instead of ItemEffect for better integration with the game system.
 * 
 * @param targeting How the item is targeted when used
 * @param effects The effects the item has when activated (now using Event directly)
...
object UsableItem {
  ...
  // Helper constructors for common item types using Events directly
  object builders {
