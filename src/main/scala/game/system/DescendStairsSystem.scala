...
/** Handles the player descending stairs to the next dungeon floor. Generates a
  * new dungeon with increased difficulty based on the current floor.
  */
object DescendStairsSystem extends GameSystem {
  override def update(
    ...
    val descendEvents = events.collect {
      case GameSystemEvent.InputEvent(entityId, InputAction.DescendStairs)
    ...
    if (descendEvents.isEmpty) {
      ...
      val worldMap = if (gameState.gameMode == game.GameMode.Gauntlet) {
        StartingState.createGauntletWorldMap(newFloor, seed)
      } else {
        val worldBounds = MapBounds(-20, 20, -20, 20)
        WorldMapGenerator.generateWorldMap(
          WorldMapConfig(
      ...
      val playerInventoryEntityIds =
        currentPlayer.inventoryItems(gameState).map(_.id).toSeq
      ...
      val newPlayer =
        currentPlayer.update[Movement](_.copy(position = newPlayerPosition))

      // Generate enemies for new floor using depth-based system with floor multiplier
      val (newEnemies, newAbilities) = {
        val dungeonRoomsOnly =
          dungeon.roomGrid.filterNot(room => room == dungeon.startPoint)

        val enemiesAndAbilities = dungeonRoomsOnly.zipWithIndex.map {
          case (roomPoint, index) =>
            if (dungeon.hasBossRoom && dungeon.endpoint.contains(roomPoint)) {
              // Boss room
              StartingState.EnemyGeneration.createEnemiesForRoom(
                roomPoint,
                Int.MaxValue,
                index,
                worldMap
              ...
              val scaledDepth =
                baseDepth + (newFloor - 1) * 3 // Add 3 depth levels per floor
              StartingState.EnemyGeneration.createEnemiesForRoom(
                roomPoint,
                scaledDepth,
                index,
      ...
      val newItems: Set[Entity] = dungeon.items.zipWithIndex.map {
        case ((point, itemReference), index) =>
          ...
          val itemEntity =
            itemReference.createEntity(s"item-floor${newFloor}-$index")
          val placedEntity =
            itemEntity.addComponent(Movement(position = basePosition))

          itemReference match {
            case data.Items.ItemReference.YellowKey =>
              placedEntity.addComponent(
                EntityTypeComponent(EntityType.Key(KeyColour.Yellow))
              )
            case data.Items.ItemReference.BlueKey =>
              placedEntity.addComponent(
                EntityTypeComponent(EntityType.Key(KeyColour.Blue))
              )
            case data.Items.ItemReference.RedKey =>
              placedEntity.addComponent(
                EntityTypeComponent(EntityType.Key(KeyColour.Red))
              )
            case _ =>
              placedEntity
          }
      ...
      val playerInventoryEntities =
        gameState.entities.filter(e => playerInventoryEntityIds.contains(e.id))
      ...
      val newGameState = GameState(
        ...
        entities = Vector(
          newPlayer
        ) ++ playerInventoryEntities ++ newItems ++ newEnemies ++ newLockedDoors ++ newAbilities.values :+ newTrader,
        worldMap = worldMap,
        dungeonFloor = newFloor,
        messages = Seq(
          s"Descended to dungeon floor $newFloor. Enemies grow stronger..."
        ),
